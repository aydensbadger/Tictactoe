...
def hard_com(com, board):
    ...
    while True:
        ...
        if board == [[0, 0, 0], [0, 0, 0], [0, 0, 0]]:
            if num1 == 1: board[0][0] = com
            elif num1 == 2: board[0][2] = com
            elif num1 == 3: board[2][0] = com
            elif num1 == 4: board[2][2] = com

            return board

        if top_R_open:
            if board[0][0] == com and board[0][1] == com:
                board[0][2] = com

                return board

        if top_L_open:
            if board[0][1] == com and board[0][2] == com:
                board[0][0] = com

                return board

        if right_open:
            if board[1][0] == com and board[1][1] == com:
                board[1][2] = com

                return board
            
        if left_open:
            if board[1][2] == com and board[1][1] == com:
                board[1][0] = com

                return board

        if bottem_R_open:
            if board[2][0] == com and board[2][1] == com:
                board[2][2] = com

                return board

        if bottem_L_open:
            if board[2][2] == com and board[2][1] == com:
                board[2][0] = com

                return board
          
        if bottem_R_open:
            if board[0][0] == com and board[1][1] == com:
                board[2][2] = com

                return board
        
        if top_L_open:
            if board[2][2] == com and board[1][1] == com:
                board[0][0] = com

                return board
        
        if bottem_L_open:
            if board[0][2] == com and board[1][1] == com:
                board[2][0] = com

                return board
        
        if top_R_open:
            if board[2][0] == com and board[1][1] == com:
                board[0][2] = com

                return board
            
        if bottem_L_open:
            if board[0][0] == com and board[1][0] == com:
                board[2][0] = com

                return board

        if top_L_open:
            if board[2][0] == com and board [1][0] == com:
                board[0][0] = com

                return board

        if bottem_open:
            if board[0][1] == com and board[1][1] == com:
                board[2][1] = com

                return board
            
        if top_open:
            if board[2][1] == com and board[1][1] == com:
                board[0][1] = com

                return board

        if bottem_R_open:
            if board[0][2] == com and board[1][2] == com:
                board[2][2] = com

                return

        if top_R_open:
            if board[2][2] == com and board[1][2] == com:
                board[0][2] = com

                return board
            
        if left_open:
            if board[0][0] == com and board[2][0] == com:
                board[1][0] = com

                return board
        
        if middle_open:
            if board[0][1] == com and board[2][1] == com:
                board[1][1] = com
                
                return board
        
        if right_open:
            if board[0][2] == com and board[2][2] == com:
                board[1][2] = com

                return board
        
        if top_open:
            if board[0][0] == com and board[0][2] == com:
                board[0][1] = com

                return board

        if middle_open:
            if board[1][0] == com and board[1][2] == com:
                board[1][1] = com

                return board
            
        if bottem_open:
            if board[2][0] == com and board[2][2] == com:
                board[2][1] = com

                return board

        if middle_open:
            if board[0][0] == com and board[2][2] == com:
                board[1][1] = com
                
                return board
        
        if middle_open:
            if board[0][2] == com and board[2][0] == com:
                board[1][1] = com

                return board

        if top_R_open:
            if board[0][0] == enemy and board[0][1] == enemy:
                board[0][2] = com

                return board

        if top_L_open:
            if board[0][1] == enemy and board[0][2] == enemy:
                board[0][0] = com

                return board

        if right_open:
            if board[1][0] == enemy and board[1][1] == enemy:
                board[1][2] = com

                return board
            
        if left_open:
            if board[1][2] == enemy and board[1][1] == enemy:
                board[1][0] = com

                return board

        if bottem_R_open:
            if board[2][0] == enemy and board[2][1] == enemy:
                board[2][2] = com

                return board

        if bottem_L_open:
            if board[2][2] == enemy and board[2][1] == enemy:
                board[2][0] = com

                return board
        
        if bottem_R_open:
            if board[0][0] == enemy and board[1][1] == enemy:
                board[2][2] = com

                return board
        
        if top_L_open:
            if board[2][2] == enemy and board[1][1] == enemy:
                board[0][0] = com

                return board
        
        if bottem_L_open:
            if board[0][2] == enemy and board[1][1] == enemy:
                board[2][0] = com

                return board
        
        if top_R_open:
            if board[2][0] == enemy and board[1][1] == enemy:
                board[0][2] = com

                return board
            
        if bottem_L_open:
            if board[0][0] == enemy and board[1][0] == enemy:
                board[2][0] = com

                return board

        if top_L_open:
            if board[2][0] == enemy and board [1][0] == enemy:
                board[0][0] = com

                return board

        if bottem_open:
            if board[0][1] == enemy and board[1][1] == enemy:
                board[2][1] = com

                return board
            
        if top_open:
            if board[2][1] == enemy and board[1][1] == enemy:
                board[0][1] = com

                return board

        if bottem_R_open:
            if board[0][2] == enemy and board[1][2] == enemy:
                board[2][2] = com

                return board

        if top_R_open:
            if board[2][2] == enemy and board[1][2] == enemy:
                board[0][2] = com

                return board
            
        if left_open:
            if board[0][0] == enemy and board[2][0] == enemy:
                board[1][0] = com

                return board
        
        if middle_open:
            if board[0][1] == enemy and board[2][1] == enemy:
                board[1][1] = com

                return board
        
        if right_open:
            if board[0][2] == enemy and board[2][2] == enemy:
                board[1][2] = com

                return board
        
        if top_open:
            if board[0][0] == enemy and board[0][2] == enemy:
                board[0][1] = com

                return board

        if middle_open:
            if board[1][0] == enemy and board[1][2] == enemy:
                board[1][1] = com

                return board
            
        if bottem_open:
            if board[2][0] == enemy and board[2][2] == enemy:
                board[2][1] = com

                return board

        if middle_open:
            if board[0][0] == enemy and board[2][2] == enemy:
                board[1][1] = com

                return board
        
        if middle_open:
            if board[0][2] == enemy and board[2][0] == enemy:
                board[1][1] = com
        ...
        if board[0][1] == enemy and board[1][0] == enemy and top_L_open == True:
            board[0][0] = com

            return board
        
        if board[0][1] == enemy and board[1][2] == enemy and top_R_open == True:
            board[0][2] = com

            return board
        
        if board[1][0] == enemy and board[2][1] == enemy and bottem_L_open == True:
            board[2][0] = com

            return board
        
        if board[2][1] == enemy and board[1][2] == enemy and bottem_R_open == True:
            board[2][2] = com

            return board

        if board[0][0] == enemy and board[2][2] == enemy or board[2][0] == enemy and board[0][2] == enemy:
            if num2 == 1: 
                if left_open: 
                    board[1][0] = com
        ...
        if board[0][0] == com and board[1][1] == com and board[0][1] == enemy and bottem_L_open:
            board[2][0] = com
...
def hard(surface, player):
    ...
    if player == 1:
        score_editor('plus', 25)
        
    if player == 2: 
        score_editor('plus', 26)
